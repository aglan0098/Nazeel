...
// ---------- Mutations ----------
function useCreateSuiteArrestAuthorities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ArrestAuthorities", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestAuthorities"] });
    },
  });
}

function useUpdateSuiteArrestAuthorities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) =>
      api.put(`/ArrestAuthorities/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestAuthorities"] });
    },
  });
...
function useDeleteSuiteArrestAuthorities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/ArrestAuthorities/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestAuthorities"] });
    },
  });
...
// ---------- Page ----------
export default function ArrestAuthorities() {
  ...
  const createMutation = useCreateSuiteArrestAuthorities();
  const updateMutation = useUpdateSuiteArrestAuthorities();
  const deleteMutation = useDeleteSuiteArrestAuthorities();

  // Columns
  const columns = [
    { key: "name", label: "الأسم" },
    { key: "lookupDisplayModeName", label: "العرض" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
          ...
          <DialogPanel className="bg-white rounded-xl shadow-xl w-auto min-w-[400px] max-h-[90%] overflow-scroll scrollbar-hide relative">
            <div className="flex justify-between items-center border-b p-5 mb-5 bg-second">
              <h2 className="text-lg font-semibold">
                {formDialog?.type === "edit" ? "تعديل جهة" : "إضافة جهة"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              <div className="md:flex gap-5 md:mb-3">
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    name="lookupDisplayModeName"
                    defaultValue={formDialog?.data?.lookupDisplayModeName || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
