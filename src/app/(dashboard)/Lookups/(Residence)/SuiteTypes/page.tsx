...
// ---------- Mutations ----------
function useCreateSuiteTypes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/SuiteTypes", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["SuiteTypes"] });
    },
  });
}

function useUpdateSuiteTypes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/SuiteTypes/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["SuiteTypes"] });
    },
  });
...
function useDeleteSuiteTypes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/SuiteTypes/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["SuiteTypes"] });
    },
  });
...
export default function SuiteTypes() {
  ...
  const createMutation = useCreateSuiteTypes();
  const updateMutation = useUpdateSuiteTypes();
  const deleteMutation = useDeleteSuiteTypes();

  // Columns
  const columns = [
    { key: "name", label: "نوع الجناح" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
