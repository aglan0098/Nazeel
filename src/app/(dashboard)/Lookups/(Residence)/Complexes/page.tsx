...
// ---------- Mutations ----------
function useCreateComplexes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/complexes", data),
    onSuccess: () => {
...
function useUpdateComplexes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/complexes/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["complexes"] });
    },
  });
...
function useDeleteComplexes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/complexes/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["complexes"] });
    },
  });
...
export default function Complexes() {
  ...
  const createMutation = useCreateComplexes();
  const updateMutation = useUpdateComplexes();
  const deleteMutation = useDeleteComplexes();

  // Columns
  const columns = [
    { key: "name", label: "المجمع" },
    { key: "description", label: "الوصف" },
    { key: "complexTypeName", label: "نوع المجمع" },
    { key: "prisonName", label: "السجن" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              <div className="md:flex gap-5 md:mb-3">
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    name="description"
                    defaultValue={formDialog?.data?.description || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
              ...
              <div className="md:flex gap-5 md:mb-3">
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    ...
                    defaultValue={formDialog?.data?.complexTypeName || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    name="prisonName"
                    defaultValue={formDialog?.data?.prisonName || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
