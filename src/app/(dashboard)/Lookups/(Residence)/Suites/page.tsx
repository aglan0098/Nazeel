...
// ---------- Mutations ----------
function useCreateSuites() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/suites", data),
    onSuccess: () => {
...
function useUpdateSuites() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/suites/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["suites"] });
    },
  });
...
function useDeleteSuites() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/suites/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["suites"] });
    },
  });
...
export default function Suites() {
  ...
  const createMutation = useCreateSuites();
  const updateMutation = useUpdateSuites();
  const deleteMutation = useDeleteSuites();

  // Columns
  const columns = [
    { key: "name", label: "الجناح" },
    { key: "suiteTypeName", label: "نوع الجناح" },
    { key: "unitName", label: "الوحدة" },
    { key: "capacity", label: "عدد الأشخاص" },
    { key: "maximumCapacity", label: "الحد الأقصي للأشخاص" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
          ...
          <DialogPanel className="bg-white rounded-xl shadow-xl w-auto min-w-[400px] max-h-[90%] overflow-scroll scrollbar-hide relative">
            <div className="flex justify-between items-center border-b p-5 mb-5 bg-second">
              <h2 className="text-lg font-semibold">
                {formDialog?.type === "edit" ? "تعديل جناح" : "إضافة جناح"}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              <div className="md:flex gap-5 md:mb-3">
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    name="suiteTypeName"
                    defaultValue={formDialog?.data?.suiteTypeName || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
              ...
              <div className="md:flex gap-5 md:mb-3">
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    name="unitName"
                    defaultValue={formDialog?.data?.unitName || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    name="capacity"
                    defaultValue={formDialog?.data?.capacity || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
              <div className="mb-4 md:mb-0">
                ...
                <input
                  ...
                  defaultValue={formDialog?.data?.maximumCapacity || ""}
                  className="w-full border p-2 rounded outline-0"
                />
              </div>
