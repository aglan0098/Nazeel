...
// ---------- Mutations ----------
function useCreateUpholdInstitution() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/UpholdInstitutions", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["UpholdInstitutions"] });
    },
  });
}

function useUpdateUpholdInstitution() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) =>
      api.put(`/UpholdInstitutions/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["UpholdInstitutions"] });
    },
  });
...
function useDeleteUpholdInstitution() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) =>
      api.delete(`/UpholdInstitutions/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["UpholdInstitutions"] });
    },
  });
...
// ---------- Page ----------
export default function UpholdInstitutions() {
  ...
  const createMutation = useCreateUpholdInstitution();
  const updateMutation = useUpdateUpholdInstitution();
  const deleteMutation = useDeleteUpholdInstitution();
