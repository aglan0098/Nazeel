...
// ---------- Mutations ----------
function useCreateReleaseType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ReleaseTypes", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ReleaseTypes"] });
    },
  });
}

function useUpdateReleaseType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/ReleaseTypes/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ReleaseTypes"] });
    },
  });
...
function useDeleteReleaseType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/ReleaseTypes/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ReleaseTypes"] });
    },
  });
...
// ---------- Page ----------
export default function ReleaseTypes() {
  ...
  const createMutation = useCreateReleaseType();
  const updateMutation = useUpdateReleaseType();
  const deleteMutation = useDeleteReleaseType();

  // Columns
  const columns = [
    { key: "name", label: "الأسم" },
    { key: "parentName", label: "النوع الفرعي" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
          ...
          <DialogPanel className="bg-white rounded-xl shadow-xl w-auto min-w-[400px] max-h-[90%] overflow-scroll scrollbar-hide relative">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="">نوع إفراج فرعي؟</label>
              </div>
