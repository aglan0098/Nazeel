...
// ---------- Mutations ----------
function useCreateArrestReason() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ArrestReasons", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestReasons"] });
    },
  });
}

function useUpdateArrestReason() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/ArrestReasons/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestReasons"] });
    },
  });
...
function useDeleteArrestReason() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/ArrestReasons/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestReasons"] });
    },
  });
...
// ---------- Page ----------
export default function ArrestReasons() {
  ...
  const createMutation = useCreateArrestReason();
  const updateMutation = useUpdateArrestReason();
  const deleteMutation = useDeleteArrestReason();
