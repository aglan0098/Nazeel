...
// ---------- Mutations ----------
function useCreateArrestDurations() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ArrestDurations", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestDurations"] });
    },
  });
}

function useUpdateArrestDurations() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) =>
      api.put(`/ArrestDurations/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestDurations"] });
    },
  });
...
function useDeleteArrestDurations() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/ArrestDurations/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ArrestDurations"] });
    },
  });
...
// ---------- Page ----------
export default function ArrestDurations() {
  ...
  const createMutation = useCreateArrestDurations();
  const updateMutation = useUpdateArrestDurations();
  const deleteMutation = useDeleteArrestDurations();

  // Columns
  const columns = [
    { key: "name", label: "الأسم" },
    { key: "parentName", label: "مفعل" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
