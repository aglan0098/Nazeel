...
// ---------- Mutations ----------
function useCreateCaseCategory() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/CaseCategories", data),
    onSuccess: () => {
...
function useUpdateCaseCategory() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) =>
      api.put(`/CaseCategories/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["CaseCategories"] });
    },
  });
...
function useDeleteCaseCategory() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/CaseCategories/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["CaseCategories"] });
    },
  });
...
export default function CaseCategories() {
  ...
  const columns = [
    ...
    { key: "subCasesCount", label: "عدد القضايا الفرعية" },
    { key: "prosecutionCode", label: "رمز النيابة" },
    { key: "parentName", label: "القضية الرئيسية" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
          ...
          <DialogPanel className="bg-white rounded-xl shadow-xl w-auto min-w-[400px] max-h-[90%] overflow-scroll scrollbar-hide relative">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              <div className="md:flex gap-5 md:mb-3">
                ...
                <div className="mb-4 md:mb-0">
                  ...
                  <input
                    ...
                    defaultValue={formDialog?.data?.prosecutionCode || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                />
                <label htmlFor="">قضية فرعية</label>
              </div>
