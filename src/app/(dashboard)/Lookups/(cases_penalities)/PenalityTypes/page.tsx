...
// ---------- Mutations ----------
function useCreatePenalityType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/PenalityTypes", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["PenalityTypes"] });
    },
  });
}

function useUpdatePenalityType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/PenalityTypes/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["PenalityTypes"] });
    },
  });
...
function useDeletePenalityTypes() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/PenalityTypes/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["PenalityTypes"] });
    },
  });
...
// ---------- Page ----------
export default function PenalityTypes() {
  ...
  const createMutation = useCreatePenalityType();
  const updateMutation = useUpdatePenalityType();
  const deleteMutation = useDeletePenalityTypes();

  // Columns
  const columns = [
    { key: "name", label: "الأسم" },
    { key: "parentName", label: "الحكم الفرعي" },
    {
      key: "actions",
      label: "#",
      render: (r: any) => {
  ...
  // handle subcase
  const [subCase, setSubCase] = useState(false);
  const handleChange = () => {
    setSubCase(!subCase);
  };
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              ...
              <div className="flex items-center gap-3">
                <input
                  className="w-5 h-5"
                  type="checkbox"
                  checked={formDialog?.data?.parentName != null}
                  onChange={handleChange}
                />
                <label htmlFor="">نوع حكم فرعي؟</label>
              </div>

              {/* {subCase && (
                <div className="flex items-center gap-3">
                  <label htmlFor="">القضية الرئيسية</label>
                  <select name="parentCase" id="parentCase">
