...
// ---------- Mutations ----------
function useCreateConfiscationType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ConfiscationTypes", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ConfiscationTypes"] });
    },
  });
}

function useUpdateConfiscationType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) =>
      api.put(`/ConfiscationTypes/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ConfiscationTypes"] });
    },
  });
...
function useDeleteConfiscationType() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/ConfiscationTypes/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ConfiscationTypes"] });
    },
  });
...
// ---------- Page ----------
export default function ConfiscationTypes() {
  ...
  const createMutation = useCreateConfiscationType();
  const updateMutation = useUpdateConfiscationType();
  const deleteMutation = useDeleteConfiscationType();
