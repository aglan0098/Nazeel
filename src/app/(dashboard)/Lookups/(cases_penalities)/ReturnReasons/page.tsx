...
// ---------- Mutations ----------
function useCreateReturnReason() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ReturnReasons", data),
    onSuccess: () => {
...
function useUpdateReturnReason() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/ReturnReasons/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ReturnReasons"] });
    },
  });
...
function useDeleteReturnReason() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/ReturnReasons/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ReturnReasons"] });
    },
  });
...
export default function ReturnReasons() {
  ...
  const createMutation = useCreateReturnReason();
  const updateMutation = useUpdateReturnReason();
  const deleteMutation = useDeleteReturnReason();
