...
// ---------- Mutations ----------
function useCreateGenericCourts() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/GenericCourts", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["GenericCourts"] });
    },
  });
}

function useUpdateGenericCourts() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) => api.put(`/GenericCourts/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["GenericCourts"] });
    },
  });
...
function useDeleteGenericCourts() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) => api.delete(`/GenericCourts/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["GenericCourts"] });
    },
  });
      ...
      {isOpen &&
        ...
        createPortal(
          <div
            ...
            className="flex justify-center items-center gap-3 bg-white border rounded-xl shadow-xl p-7"
          >
            <button
...
// ---------- Page ----------
export default function GenericCourts() {
  ...
  const createMutation = useCreateGenericCourts();
  const updateMutation = useUpdateGenericCourts();
  const deleteMutation = useDeleteGenericCourts();
          ...
          <DialogPanel className="bg-white rounded-xl shadow-xl w-auto min-w-[400px] max-h-[90%] overflow-scroll scrollbar-hide relative">
            <div className="flex justify-between items-center border-b p-5 mb-5 bg-second">
              <h2 className="text-lg font-semibold">
                {formDialog?.type === "edit" ? "تعديل محكمة" : "إضافة محكمة "}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-7">
              <div className="md:flex gap-5 md:mb-3">
                ...
                <div className="my-5">
                  ...
                  <input
                    name="location"
                    defaultValue={formDialog?.data?.location || ""}
                    className="w-full border p-2 rounded outline-0"
                  />
                </div>
              </div>
