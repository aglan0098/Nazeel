...
// ---------- Mutations ----------
function useCreateSuiteExternalAuthorities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: any) => api.post("/ExternalAuthorities", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ExternalAuthorities"] });
    },
  });
}

function useUpdateSuiteExternalAuthorities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, ...data }: any) =>
      api.put(`/ExternalAuthorities/${id}`, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ExternalAuthorities"] });
    },
  });
...
function useDeleteSuiteExternalAuthorities() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (id: string | number) =>
      api.delete(`/ExternalAuthorities/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["ExternalAuthorities"] });
    },
  });
...
// ---------- Page ----------
export default function ExternalAuthorities() {
  ...
  const createMutation = useCreateSuiteExternalAuthorities();
  const updateMutation = useUpdateSuiteExternalAuthorities();
  const deleteMutation = useDeleteSuiteExternalAuthorities();
